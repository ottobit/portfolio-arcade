...
test('stationary car cannot pivot; reverse reverses yaw; fast steering stays responsive but controlled',()=>{const low=Math.abs(steeringYaw(1,20,2,1));const mid=Math.abs(steeringYaw(1,50,2,1));const top=Math.abs(steeringYaw(1,84,2,1));assert.equal(Math.abs(steeringYaw(1,0,2,1)),0);assert.equal(steeringYaw(1,20,2,1),-steeringYaw(1,-20,2,1));assert(mid>low*.65);assert(top>low*.5);assert(top<low*.65);});
