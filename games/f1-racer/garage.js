...
car.traverse(o=>{
 if(o.name==='floorPanel')o.scale.x=setup.floor==='high'?1.08:setup.floor==='low'?.94:1;
 if(o.name==='diffuserFin')o.scale.y=setup.floor==='high'?1.5:setup.floor==='low'?.6:1;
 if(o.name==='setupSpring')o.scale.y=setup.suspension==='soft'?1.2:setup.suspension==='stiff'?.75:1;
 if(o.name==='setupCaliper')o.material.color.set(setup.brakes==='aggressive'?0xd54b38:setup.brakes==='stable'?0x74b6c7:0xb69050);
});const fw=car.getObjectByName("frontWing"),rw=car.getObjectByName("rearWing");if(fw)fw.rotation.x=setup.frontWing==="high"?-.16:setup.frontWing==="low"?.08:-.05;if(rw){rw.position.y=setup.rearWing==="high"?1.08:setup.rearWing==="low"?.86:.95;rw.rotation.x=setup.rearWing==="high"?-.18:setup.rearWing==="low"?.08:0}}
const labels={speed:"Velocità",downforce:"Carico",braking:"Frenata",stability:"Stabilità",traction:"Trazione"};
function renderUI(){const effects=setupEffects(setup),base={speed:50,downforce:50,braking:50,stability:50,traction:50};document.getElementById("garage-stats").innerHTML=Object.entries(base).map(([k,v])=>`<div><span>${labels[k]}</span><div><i style="width:${Math.max(10,Math.min(90,v+(effects[k]||0)*7))}%"></i></div></div>`).join("");document.getElementById("garage-parts").innerHTML=Object.entries(GARAGE_PARTS).map(([part,data])=>`<section class="garage-part"><h2>${data.label}</h2><div>${Object.entries(data.variants).map(([id,v])=>`<button draggable="true" data-part="${part}" data-id="${id}" aria-pressed="${setup[part]===id}" class="${setup[part]===id?"active":""}">${v.label}</button>`).join("")}</div></section>`).join("");document.querySelectorAll(".garage-mount").forEach(z=>z.classList.toggle("installed",!!setup[z.dataset.part]));applyVisual()}
function mount(part,id){if(!GARAGE_PARTS[part]?.variants[id])return;setup[part]=id;saveGarageSetup(setup);renderUI();focusPart(part);const z=document.querySelector(`.garage-mount[data-part="${part}"]`);if(z){z.classList.add("just-mounted");setTimeout(()=>z.classList.remove("just-mounted"),650)}document.getElementById("garage-status").textContent=`${GARAGE_PARTS[part].label}: ${GARAGE_PARTS[part].variants[id].label} montato.`}
