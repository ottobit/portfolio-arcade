...
export function surfaceTexture(kind,renderer){
  ...
  for(let i=0;i<data.data.length;i+=4){const grain=rand()*24;const grass=kind==='grass';data.data[i]=(grass?88:49)+grain;data.data[i+1]=(grass?108:51)+grain;data.data[i+2]=(grass?66:55)+grain;data.data[i+3]=255;}
  ...
  if(kind==='grass')texture.repeat.set(180,180);
  texture.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());return texture;
}

export function dressCircuit(scene,points,width,renderer,wet){
  ...
  const white=material(0xebe7d9),runoff=material(wet?0x315452:0x467f72);
  for(const side of [-1,1]){ribbon(side*(half+1.65),2.8,.006,runoff);ribbon(side*(half-.18),.14,.025,white);ribbon(side*(half+3.02),.13,.014,white);}
